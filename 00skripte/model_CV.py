...
for mm in range(len(modelvars)):
    modelvar = modelvars[mm]
    ...
    # paths
    pth_cwd = "D:/06a_Paper Globales Modell/x_Corrigendum/"
    # pth_cwd = "/pfs/work7/workspace/scratch/gw3013-corrigendum/"
    os.chdir(pth_cwd)
    pth_dt_gw = "./01data/gw"
    pth_dt_meteo = "./01data/hyras"
    
    if modelvar == "onlydyn_lstm":
        pth_out = "./02ergebnisse/ablation_onlydyn_CV"   
    if modelvar == "envfeat_lstm":
        pth_out = "./02ergebnisse/envfeat_CV"   
    if modelvar == "tsfeat_lstm":
        pth_out = "./02ergebnisse/tsfeat_CV"
    if modelvar == "envfeat_cnn":
        pth_out = "./02ergebnisse/envfeat_CNN_CV"
    if modelvar == "tsfeat_cnn":
        pth_out = "./02ergebnisse/tsfeat_CNN_CV"
    if modelvar == "rndfeat9_lstm":
        pth_out = "./02ergebnisse/rndfeat9_CV"
    if modelvar == "rndfeat18_lstm":
        pth_out = "./02ergebnisse/rndfeat18_CV"
    ...
    def preprocessing(meteo_in,gw_in,IDs,n_input):
        ...
        for i_internal in range(len(meteo_in)):
            
            # merge all input data
            tempdata = copy.deepcopy(meteo_in[i_internal])
            tempdata["ID"] = i_internal
            tempdata = pd.merge(tempdata, gw_in[i_internal], on = "Date", how = "inner")
            ...
            tempdata_n = scalers[i_internal].transform(tempdata)
            ...
            IDlen.append(np.repeat(i_internal,len(tempdata_n_test)))
            ...
            temp_x, temp_y = make_sequences(np.asarray(tempdata_n_train), n_steps_in, n_input = n_input)
            X_train.append(temp_x); Y_train.append(temp_y[:,1])
            X_train_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == IDs[i_internal]], 
                                          len(temp_x), axis = 0))
            
            temp_x, temp_y = make_sequences(np.asarray(tempdata_n_stop_ext), n_steps_in, n_input = n_input)
            X_stop.append(temp_x); Y_stop.append(temp_y[:,1])
            X_stop_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == IDs[i_internal]], 
                                         len(temp_x), axis = 0))
            
            temp_x, temp_y = make_sequences(np.asarray(tempdata_n_test_ext), n_steps_in, n_input = n_input)
            X_test.append(temp_x); Y_test.append(temp_y[:,1])
            X_test_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == IDs[i_internal]], 
                                         len(temp_x), axis = 0))
        ...
        output = [X_train,X_train_stat,Y_train,
                  X_stop,X_stop_stat,Y_stop,
                  X_test,X_test_stat,Y_test,
                  IDlen,datafull,datafullwide,scalers_y]
        return output
    ...
    fltr2 = fltr.reset_index(drop = True)
    ...
    # jj=0
    for jj in range(len(HP_seeds)):
    
        HPi_seed = HP_seeds[jj]
        
        # create folder for outputs
        pth_out_i = pth_out+'/run'+str(jj)
        os.mkdir(pth_out_i)
        ...
        # define startdate of test period
        date_start_test = pd.to_datetime("2012-01-01", format = "%Y-%m-%d")
        
        # define k (for k-fold CV) and number of samples in k
        k = 10
        k_n = int(np.round(len(dt_list_gw)/k))
        
        # sample k times without replacement
        enumlen = np.arange(0,len(dt_list_gw))
        samples = list()
        for i in range(k-1):
            samples.append(np.random.choice(enumlen, k_n, replace=False))
            enumlen = enumlen[[item not in samples[i] for item in enumlen]]
        samples.append(enumlen)
        ...
        for ii in range(k):
            ...
            # split into train and leave-out set
            meteo_in_LI = copy.deepcopy(dt_list_meteo)
            gw_in_LI = copy.deepcopy(dt_list_gw)
            
            # separate leave-out sample
            meteo_in_LO = [meteo_in_LI[item] for item in samples[ii]]
            gw_in_LO = [gw_in_LI[item] for item in samples[ii]]
            dt_list_names_LO = [dt_list_names[item] for item in samples[ii]]
            
            #reduce train set by leave-out sample
            meteo_in_LI = [i for j, i in enumerate(meteo_in_LI) if j not in samples[ii]]
            gw_in_LI = [i for j, i in enumerate(gw_in_LI) if j not in samples[ii]]
            dt_list_names_LI = [i for j, i in enumerate(dt_list_names) if j not in samples[ii]]
            
            # data preprocessing: train set / leave-in set
            X_train_LI,X_train_stat_LI,Y_train_LI,X_stop_LI,X_stop_stat_LI,Y_stop_LI,X_test_LI,X_test_stat_LI,Y_test_LI,IDlen_LI,datafull_LI,datafullwide_LI,scalers_y_LI = preprocessing(meteo_in=meteo_in_LI,gw_in=gw_in_LI,IDs=dt_list_names_LI,n_input=4)
            
            # data preprocessing: leave-out set
            X_train_LO,X_train_stat_LO,Y_train_LO,X_stop_LO,X_stop_stat_LO,Y_stop_LO,X_test_LO,X_test_stat_LO,Y_test_LO,IDlen_LO,datafull_LO,datafullwide_LO,scalers_y_LO = preprocessing(meteo_in=meteo_in_LO,gw_in=gw_in_LO,IDs=dt_list_names_LO,n_input=4)
            ...
            # Callbacks
            model_mc = tf.keras.callbacks.ModelCheckpoint(filepath=pth_out_i+'/model_k'+str(ii), 
                                                          save_best_only=True)
            model_wcd = WarmupCosineDecay(total_steps=len(X_train_LI)/HPi_batchsize*HPi_epochs, 
                                          warmup_steps=int(len(X_train_LI)/HPi_batchsize),
                                          hold=0, 
                                          target_lr=HPi_targetlr)
            
            if modelvar == "onlydyn_lstm":
                # Input layers for dynamic and static model strands
                model_dyn_in = tf.keras.Input(shape=(n_steps_in, X_train_LI.shape[2]))
                ...
                model_history = model.fit([X_train_LI], Y_train_LI, 
                                          validation_data=([X_stop_LI], Y_stop_LI),
                                          epochs=HPi_epochs, verbose=2, batch_size=HPi_batchsize, 
                                          shuffle=True, callbacks=[model_mc,model_wcd])
            
            if modelvar in ["tsfeat_lstm","envfeat_lstm","rndfeat9_lstm","rndfeat18_lstm"]: 
                # Input layers for dynamic and static model strands
                model_dyn_in = tf.keras.Input(shape=(n_steps_in, X_train_LI.shape[2]))
                model_stat_in = tf.keras.Input(shape=(X_train_stat_LI.shape[1],))
                # Dynamic model strand
                model_dyn = tf.keras.layers.LSTM(HPi_lstm_size, activation='relu')(model_dyn_in)
                model_dyn = tf.keras.layers.Dropout(HPi_dropout)(model_dyn)
                # Static model strand
                ...
                # Train model on training data
                model_history = model.fit([X_train_LI, X_train_stat_LI], Y_train_LI, 
                                          validation_data=([X_stop_LI, X_stop_stat_LI], Y_stop_LI),
                                          epochs=HPi_epochs, verbose=2, batch_size=HPi_batchsize, 
                                          shuffle=True, callbacks=[model_mc,model_wcd])
            ...
            if modelvar == "onlydyn_lstm":
                ...
                loaded_model = tf.keras.models.load_model(pth_out_i+'/model_k'+str(ii))
                sim_n = loaded_model.predict([X_test_LO])
            
            if modelvar in ["tsfeat_lstm","envfeat_lstm","rndfeat9_lstm","rndfeat18_lstm","tsfeat_cnn","envfeat_cnn"]: 
                # predict - with saved model checkpoint
                loaded_model = tf.keras.models.load_model(pth_out_i+'/model_k'+str(ii))
                sim_n = loaded_model.predict([X_test_LO, X_test_stat_LO])
            
            # inverse scaling
            sim = []
            for i in range(len(meteo_in_LO)):
                temp = sim_n[IDlen_LO == i,0]
                temp = scalers_y_LO[i].inverse_transform(temp.reshape(-1, 1))[:,0]
            ...
            for i in range(len(meteo_in_LO)):
                temp = sim[sim.ID == i]
                temp.columns = (dt_list_names[samples[ii][i]] + "_" + temp.columns).tolist()
                temp[dt_list_names[samples[ii][i]]+"_obs"] = scalers_y_LO[i].inverse_transform(Y_test_LO[IDlen_LO == i].reshape(-1, 1))[:,0]
            ...
            sim_test.columns = np.arange(0,len(meteo_in_LO))
            obs_test = results.filter(like = "obs")
            obs_test.columns = np.arange(0,len(meteo_in_LO))
        
            err_test = sim_test-obs_test
            err_nash = obs_test - np.mean(datafullwide_LO[datafullwide_LO.index < date_start_test], 
            ...
            scores.index = [dt_list_names[item] for item in samples[ii]]
            ...
            # Scores
            scores.to_csv(pth_out_i+'/scores_k'+str(ii)+'.csv',
                          float_format='%.3f', sep = ";")
            
            # train history
            pd.DataFrame(model_history.history).to_csv(pth_out_i+'/losshistory_k'+str(ii)+'.csv', float_format='%.4f', sep = ";")
        
            # export obs+sim data of test period
            results.to_csv(pth_out_i+'/results_k'+str(ii)+'.csv', float_format='%.4f', sep = ";")
