...
def preprocessing(meteo_in,gw_in,n_input):
    ...
    for i_internal in range(len(meteo_in)):
        ...
        X_train_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == dt_list_names[i_internal]], 
                                      len(temp_x), axis = 0))
        ...
        X_stop_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == dt_list_names[i_internal]], 
                                     len(temp_x), axis = 0))
        ...
        X_test_stat.append(np.repeat(dt_static_features_n[dt_static_features.index == dt_list_names[i_internal]], 
                                     len(temp_x), axis = 0))
...
for jj in range(len(HP_seeds)):
    ...
    for ii in range(k):
        ...
        X_train_LI,X_train_stat_LI,Y_train_LI,X_stop_LI,X_stop_stat_LI,Y_stop_LI,X_test_LI,X_test_stat_LI,Y_test_LI,IDlen_LI,datafull_LI,datafullwide_LI,scalers_y_LI = preprocessing(meteo_in=meteo_in_LI,gw_in=gw_in_LI,n_input=4)
        
        # data preprocessing: leave-out set
        X_train_LO,X_train_stat_LO,Y_train_LO,X_stop_LO,X_stop_stat_LO,Y_stop_LO,X_test_LO,X_test_stat_LO,Y_test_LO,IDlen_LO,datafull_LO,datafullwide_LO,scalers_y_LO = preprocessing(meteo_in=meteo_in_LO,gw_in=gw_in_LO,n_input=4)
        ...
        for i in range(len(meteo_in_LO)):
            ...
            temp = scalers_y_LO[i].inverse_transform(temp)
        ...
        for i in range(len(meteo_in_LO)):
            ...
            temp[dt_list_names[samples[ii][i]]+"_obs"] = scalers_y_LO[i].inverse_transform(Y_test_LO[IDlen_LO == i])
